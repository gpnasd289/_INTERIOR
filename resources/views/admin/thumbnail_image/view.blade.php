    ...
    <div class="row">
        <div class="col-sm-12">
            <div class="page-title-box">
                <h4 class="page-title">Manage Webtie</h4>
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="javascript:void(0);">Dashboard</a></li>
                    <li class="breadcrumb-item active"><a href="javascript:void(0);">Webtie</a></li>
                </ol>
            </div>
        </div>
    </div>
    <x-cardbody title="Manage Webtie">
            <table id="datatable" class="table table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                ...
                <tbody>
                    @foreach($list as $line)
                        <tr id="r-{{ $line -> ID}}">
                            <td>{{ $line-> ID }}</td>
                            <td> <div class="d-flex justify-content-center"><img src="{{ $line -> file_path }}" height="300px" > </div></td>
                            <td>{{ $line-> status }}</td>
                            <td>{{ $line-> position }}</td>
                            <td>{{ $line-> created_at() }}</td>
                            <td>{{ $line-> updated_at() }}</td>
                            <td><x-row-function :item="$line" :prefix="$route_prefix"> </x-row-function></td>
