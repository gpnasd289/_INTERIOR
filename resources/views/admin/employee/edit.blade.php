...
@section('main-content')
    ...
    <x-cardbody title="Edit Employee">
        <form class="form" action="{{ route('admin.employee.update') }}" method="POST" enctype="multipart/form-data">
            ...
            <x-inputtext title="Name" name="name" value="{{ old('name', $data->profile->name)}}" placeholder="Enter employee name ..." id="name"></x-inputtext>
            @if($errors->has('name'))
                <div class="error"> {{ $errors->first('name') }} </div>
            @endif
            <div class="row">
                <div class="col-sm">
                <x-inputdate title="Date Of Birth" name="date" value="{{ old('date', $data->profile->date_of_birth)}}" id="date"></x-inputdate>
                @if($errors->has('date'))
                <div class="error"> {{ $errors->first('date') }} </div>
                @endif
                </div>
                <div class="col-sm">
                    <x-inputtext title="Address" name="address" value="{{ old('address', $data->profile->address)}}" placeholder="Enter your address ..." id="address"></x-inputtext>
                    @if($errors->has('address'))
                        <div class="error"> {{ $errors->first('address') }} </div>
                    @endif
                </div>
                <div class="col-sm">
                    <x-inputtext title="Born Place" name="born_place" value="{{ old('born_place', $data->profile->born_place)}}" placeholder="Enter your born place ..." id="born_place"></x-inputtext>
            ...
            <div class="row">
                <div class="col-sm">
                    <x-inputtext title="CCID" type="number" name="ccid" value="{{ old('ccid', $data->profile->CCID)}}" placeholder="Enter your CCID" id="ccid"></x-inputtext>  
                    @if($errors->has('ccid'))
                        <div class="error"> {{ $errors->first('ccid') }} </div>
                    @endif
                </div>
                <div class="col-sm">
                    <x-inputtext title="Phone Number" type="number" name="phonenumber" value="{{ old('phonenumber', $data->profile->phone_number)}}" placeholder="Enter your Phone Number ..." id="phonenumber"></x-inputtext>
                    @if($errors->has('phonenumber'))
                        <div class="error"> {{ $errors->first('phonenumber') }} </div>
                    @endif
                </div>
                <div class="col-sm">
                    <x-inputtext title="Email" name="email" type="email" value="{{ old('email', $data->profile->email)}}" placeholder="youemail@gmail.com" id="email"></x-inputtext>
                    @if($errors->has('email'))
            ...
            <x-inputtext title="Username" name="username" value="{{ old('username', $data->username)}}" placeholder="Enter new username" id="username"></x-inputtext>
            @if($errors->has('username'))
                <div class="error"> {{ $errors->first('username') }} </div>
            @endif
            <x-inputtext title="Role" name="role" value="{{ old('role', $data->role)}}" placeholder="Enter role" id="role"></x-inputtext>
            @if($errors->has('role'))
