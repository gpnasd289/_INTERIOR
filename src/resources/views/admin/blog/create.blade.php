...
@section('main-content')
    ...
    <x-cardbody title="Tạo mới Blog">
        <form class="form" action="{{ route('admin.blog.store') }}" method="POST" enctype="multipart/form-data">
            ...
            @endif
            <x-inputtext title="Tiêu đề" name="title" value="{{ old('title', '')}}" placeholder="Nhập tiêu đề bài viết" id="title"></x-inputtext>
            @if($errors->has('title'))
                <div class="error"> {{ $errors->first('title') }} </div>
            @endif
            <x-inputtext title="Mô tả bài viết" name="description" value="{{ old('description', '')}}" placeholder="Nhập mô tả bài viết" id="description"></x-inputtext>
