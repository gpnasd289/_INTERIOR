...
@section('main-content')
    ...
    <x-cardbody title="Tạo mới mã giảm giá">
        <form class="form" action="{{ route('admin.promotion.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
             <x-inputtext title="Mã giảm giá" name="title" value="{{ old('title',  $promotion -> title  )}}" placeholder="Nhập mã giảm giá ví dụ: CHAOMUNG2022MINN19" id="title"></x-inputtext>       
             @if($errors->has('title'))
                <div class="error"> {{ $errors->first('title') }} </div>
            @endif 
            <x-inputtext title="Mô tả" name="description" value="{{ old('description',  $promotion -> description )}}" placeholder="Nhập mô tả mã giảm giá" id="description"></x-inputtext>
            ...
            <div class="row">
                ...
                <label  class="w-100" for="promotion_apply">
                    ...
                    @else 
                        <input type="radio" name="promotion_apply"  value="1"  />  Cho toàn bộ sản phẩm
                    @endif
                </label>
                <br>
                <label  class="w-100" for="promotion_apply">
                @include('admin.promotion.radiobutton_component', [ 'condition' => $promotion -> promotion_apply,
                                                                    'value' => 2,
                                                                    'name' => 'promotion_apply',
                                                                    'title' => 'Theo danh mục sản phẩm',
                                                                    'name_value' => 'promotion_category_apply_value'])
                ...
                @include('admin.promotion.radiobutton_component', [ 'condition' => $promotion -> promotion_apply,
                                                                    'value' => 3,
                                                                    'name' => 'promotion_apply',
                                                                    'title' => 'Theo sản phẩm cụ thể',
                                                                    'name_value' => 'promotion_product_apply_value'])
            ...
            <div class="form-group">
                ...
                <label class="w-60">
                    @if($promotion -> promotion_condition == 0)
                        <input type="radio" class="promotion_condition" name="promotion_condition" checked value="0"  />  Không cần điều kiện   
                    @else 
                        <input type="radio" class="promotion_condition" name="promotion_condition"  value="0"  />  Không cần điều kiện   
                    @endif
               ...
               <label  class="w-60">
                    @if ($promotion -> promotion_condition == 1 )
                        <input type="radio" class="promotion_condition" name="promotion_condition"  value="1" checked />  Tổng giá trị đơn hàng thấp nhất
                        <div class="lowest-total-bill">
                            <input type="number" name="condition_value_lowest_bill" class="form-control" value="{{ old('condition_value_lowest_bill',$promotion -> condition_value) }}" placeholder="100000, 200000 VND">
                        </div>
                    @else 
                ...
                <br>
                <label  class="w-60">
                    @if($promotion -> promotion_condition == 2) 
                        <input type="radio" class="promotion_condition" name="promotion_condition"  value="2" checked />  Giới hạn số tiền giảm giá
                        <div class="minimum-money-discount">
                            <input type="number" name="condition_value_minimum_money" class="form-control" value="{{ old('condition_value_minimum_money', $promotion -> condition_value) }}" placeholder="100000, 200000 VND">
                        </div>
                    @else 
                ...
                <br>
                <label>
                    @if($promotion -> promotion_condition == 3) 
                        <input type="radio" class="promotion_condition" name="promotion_condition"  value="3" checked />  Giới hạn số lượng sản phẩm trong một đơn hàng
                        <div class="minimum-product">
                            <input type="number" name="condition_value_minimum_product" class="form-control" value="{{ old('condition_value_minimum_product', $promotion -> condition_value) }}" placeholder="100000, 200000 VND">
                        </div>
                    @else 
            ...
            <label for="start_date" class="control-label" required>Ngày bắt đầu</label>
                ...
                <div class="form-group w-60" style="width:300px;">
                        <input type="date" class="form-control" value="{{ old('start_date', date('Y-m-d',  strtotime($promotion -> start_date)))  }}" placeholder="{{ old('end_date',  $promotion -> start_date) }}" id="start_date" name="start_date">
                    </div>
            ...
            <div class="form-group">
                ...
                <div>
                    <div class="form-group w-60" style="width:300px;">
                        <input type="date" class="form-control" value="{{ old('end_date',  date('Y-m-d',  strtotime($promotion -> end_date))) }}" placeholder="{{ old('end_date',  $promotion -> end_date) }}" id="end_date" name="end_date">
                    </div>
            ...
            <x-inputtext title="Số lượng mã giảm giá" name="quantity" value="{{ old('quantity', $promotion -> quantity)}}" type="number" placeholder="Nhập số lượng mã giảm giá"></x-inputtext>
            <div class="form-group">
                ...
                <div class="form-group">
                    @if($promotion -> discount_type == 1) 
                        ...
                        <input type="radio"  name="discount_type"  value="1" checked />  Giảm theo phần trăm (%) 
                    </label>
                    <br>
                    <label  class="w-60">
                   ...
                   <br>
                   <label  class="w-60">
                       <input type="radio"  name="discount_type"  value="2" checked />  Giảm giá tiền
                   </label>
                    @endif
                </div>
          
                <input type="number" name="discount_value" class="form-control" value="{{ old('discount_value',$promotion -> discount_value ) }}" placeholder="mức giảm giá">
            </div>
            <div class="form-group">
                ...
                <div>
                    ...
                    <label class="w-60">
                        ...
                        @else
                            <input type="radio" class="" name="status"  value="1"  />  Hoạt động   
                        @endif 
                     ...
                     <label class="w-60">
                        @if($promotion -> status == 0) 
                            <input type="radio" class="" name="status"  value="0" checked />  Ngưng hoạt động 
                        @else 
